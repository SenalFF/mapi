...
app.get('/download', async (req, res) => {
  try {
    ...
    // Extract links from buttons based on provided HTML and Screenshot
    $('.wait-done a, #dl-links a, .download-section a, .button, a#link').each((i, el) => {
      const $btn = $(el);
      const href = $btn.attr('href');
      const text = $btn.text().trim().toLowerCase();
      if (!href || href.startsWith('#') || href.includes('javascript:')) return;

      if (text.includes('direct') || href.includes('sonic-cloud') || href.includes('google.com/server')) {
    ...
    res.json({
      ...
      file_info: { name: fileName, size: fileSize }
    });
  } catch (error) {
    console.error('Download error:', error.message);
    res.status(500).json({ error: 'Download resolution failed', message: error.message });
  }
});
